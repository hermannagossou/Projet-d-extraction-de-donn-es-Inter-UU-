...
if uploaded_file is not None:
    ...
    BPE=[]
    ELEMENT=[]
    BPENO=['BPESOUI','BPESNON','BPEDOUI','BPEDNON','BPEAOUI','BPEANON','FOA','Données','JD','Passage']
    ...
    for frame in df_dict:
        ...
        if frame in BPENO:
            ...
            BPE.append(PDS[PDS.str.contains('^BD[0-9]{6}|BPEU[0-9]|BPEA[0-9]|BPEI[0-9]|BPI[0-9]|BPP[0-9]',na=False)].iloc[0])
            ...
            TYPE_CHAINE=PDS[PDS.str.contains('HD\s*$|FR6\s*$|Taille 1\s*$|Taille 0\s*$|3M T1\s*$|3M T0\s*$|BPEO T1\s*',na=False)]
